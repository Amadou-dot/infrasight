 ...
 */
const RATE_LIMIT_EXEMPT_PATHS = ['/api/health', '/api/v2/metrics', '/api/v2/health'];
 ...
 */
export function getRateLimitConfig(path: string, method: string): EndpointLimits | null {
  // Check if rate limiting is enabled
  if (!isRateLimitEnabled()) return null;

  // Check if path is exempt
  if (isRateLimitExempt(path)) return null;

  // Check for exact path match first
  if (RATE_LIMIT_CONFIGS[path]) return RATE_LIMIT_CONFIGS[path];
