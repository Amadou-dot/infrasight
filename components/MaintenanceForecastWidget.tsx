...
export default function MaintenanceForecastWidget({
  ...
  const getDeviceIssue = (device: DeviceV2Response, severity: Section): string => {
    ...
    if (device.metadata.next_maintenance) {
      ...
      if (isPast) issues.push(`Maintenance overdue ${formatRelativeDate(maintenanceDate)}`);
      else issues.push(`Maintenance ${formatRelativeDate(maintenanceDate)}`);
    }
      ...
      <CardContent className="space-y-4">
        ...
        {forecast.summary.avg_battery_all !== null && forecast.summary.avg_battery_all < 50 && (
          <div className="flex items-center gap-2 text-sm text-amber-600 dark:text-amber-400">
            ...
            <span>Average battery: {Math.round(forecast.summary.avg_battery_all)}%</span>
          </div>
        )}
      </CardContent>
    </Card>
