...
export async function GET(request: NextRequest) {
  ...
  return withErrorHandler(async () => {
    ...
    const validationResult = validateQuery(searchParams, maintenanceForecastQuerySchema);
    ...
    const response = await getOrSet(
      ...
      async () => {
        ...
        const now = new Date();
        const criticalMaintenanceDate = new Date(now.getTime() + 3 * 24 * 60 * 60 * 1000); // 3 days
        const warningMaintenanceDate = new Date(
          now.getTime() + query.days_ahead * 24 * 60 * 60 * 1000
        );
        const warrantyWarningDate = new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000); // 30 days
        ...
        for (const device of allDevices) {
          ...
          if (device.metadata.next_maintenance && new Date(device.metadata.next_maintenance) < now)
            isCritical = true;
        ...
        const maintenanceOverdue = critical.filter(
          d => d.metadata.next_maintenance && new Date(d.metadata.next_maintenance) < now
        );

        // Calculate average battery level
        const devicesWithBattery = allDevices.filter(d => d.health.battery_level !== undefined);
        const avgBatteryAll =
          devicesWithBattery.length > 0
            ? devicesWithBattery.reduce((sum, d) => sum + (d.health.battery_level || 0), 0) /
