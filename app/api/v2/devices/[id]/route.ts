...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return withErrorHandler(async () => {
    ...
    if (device.audit?.deleted_at)
      throw new ApiError(ErrorCodes.NOT_FOUND, 410, `Device '${id}' has been deleted`, {
        deleted_at: device.audit.deleted_at,
        deleted_by: device.audit.deleted_by,
      });
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return withErrorHandler(async () => {
    ...
    if (!existingDevice) throw ApiError.notFound('Device', id);

    // Check if device is soft-deleted
    if (existingDevice.audit?.deleted_at)
      throw new ApiError(ErrorCodes.NOT_FOUND, 410, `Cannot update deleted device '${id}'`, {
...
async function handleDeleteDevice(
  ...
  return withErrorHandler(async () => {
    ...
    if (device.audit?.deleted_at)
      throw new ApiError(ErrorCodes.NOT_FOUND, 410, `Device '${id}' is already deleted`, {
        deleted_at: device.audit.deleted_at,
      });
